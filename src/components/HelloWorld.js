...
const Box = function ({style, className = '', ...rest}) {
  ...
  // }
  return (
    <div className={`box ${className}`} style={{...styling, ...style}} {...rest}></div>
  )
}
...
const element = (
  <main {...props} className={`big ${this.nightMode ? 'night-mode' : ''}`}>
     ...
     <div className="action-bar">
       <Box className="box--small" style={{backgroundColor: 'lightBlue', fontSize: '1em'}}>Lily Box</Box>
       <Box className="box--medium" style={{backgroundColor: 'pink', fontSize: '1.5em'}}>Sara Box</Box>
       <Box className="box--large" style={{backgroundColor: 'orange', fontSize: '2em'}}>David Box</Box>
     </div>
