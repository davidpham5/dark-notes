...
class Weather extends Component {
  ...
  componentWillReceiveProps (nextProps) {
    if (nextProps.location) {
      this.showWeather(nextProps.location).then((resp) => {
  ...
  render () {
    ...
    if (!forecast.currently) { return false }

    function formatAMPM (date) {
      const d = new Date(date * 1000)
      const formattedTime = d.toLocaleString('en-US', { hour12: true, hour: '2-digit', minute: '2-digit' })
